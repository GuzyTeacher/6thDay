...
//document.body.onmousedown = function() {return false; } //so page is unselectable
	...
	var nPanel = 0
	var mapGrid = []
	for(var i =0; i < Math.floor(w*2/100); i++){
		mapGrid[i] = [];
		for(var j =0; j < Math.floor(w*2/100); j++){
	...
	var steamSound = []
	for(var i=0; i < 5; i++)steamSound.push(addSound('Sounds/steam1.ogg', false));

	var pulseLightSound = []
	for(var i=0; i < 10; i++)pulseLightSound.push(addSound('Sounds/pulseLight4.ogg', false));
	
	var florSound = []
	for(var i=0; i < 10; i++)florSound.push(addSound('Sounds/florLight.ogg', false));
	
	var spinSound = []
	for(var i=0; i < 5; i++)spinSound.push(addSound('Sounds/spinLight.ogg', false));

	var sparkSound = []
	for(var i=0; i < 5; i++)sparkSound.push(addSound('Sounds/sparks.ogg', false));
	
	var flickerSound = []
	for(var i=0; i < 10; i++)flickerSound.push(addSound('Sounds/flickerLight2.ogg', false));
	...
	wallFeatures.push(makePicture('Animations/Objects/Features/monitor.png'));
	wallFeatures.push(makePicture('Animations/Objects/Features/monitor1.png'));
	wallFeatures.push(makePicture('Animations/Objects/Features/monitor2.png'));
	wallFeatures.push(makePicture('Animations/Objects/Features/monitor3.png'));
	wallFeatures.push(makePicture('Animations/Objects/Features/monitor4.png'));
	wallFeatures.push(makePicture('Animations/Objects/Features/monitor5.png'));
...
mainMenuLevel.wallPanels.push(new wordWall(750,560, 'BY ADAM GUZY',5));
	...
	function CargoElevator(a,b,c,d){
		...
		this.draw = function(){
			...
			if (this.ty < this.gy) {
				...
				this.light1.ly+=2;
				this.light2.y+=2;
				for(var i=0; i < this.lights.length; i++) this.lights[i].ly+=2;
				for(var i=0; i < this.panels.length; i++) this.panels[i].y+=2
	...
	function BigLamp(a,b){
		...
		this.on = true
		this.distance = 0
		this.sound = florSound[rand(florSound.length)]
		this.draw = function(){
			if(this.on){
				ctx.drawImage(biglampLit, this.lx,this.ly);
				lightRegion(this.lx + 40, this.ly + 100, 12, 1.5);
				
				//ctx.fillText(this.sound.soundVar.currentTime, this.lx, this.ly)
			...
			}else ctx.drawImage(biglampOut, this.lx,this.ly);
		}
	}
	...
	function FlickerLamp(a,b){
		...
		this.on = true;
		this.distance = 0;
		this.sound = flickerSound[rand(flickerSound.length)]
	...
	//	paray = (creature[0].y - h/2)*0.1 + ctxOy*5
		...
		*/
		...
		if(!mDown){
			...
			if(dPx < -10){
				ctxOx += pans - dPx/30
			}else if(dPx > 10)	ctxOx -= pans + dPx/30
	
			if(dPy < -10){
				ctxOy += pans - dPy/30
			}else if(dPy > 10)	ctxOy -= pans + dPy/30
		...
		if(mDown){
			if(mx > mapX && mx < mapX + mapW && my > mapY && my < mapY + mapH){
				...
				if(creature[cS].fireCool <= 0 && creature[cS].canFire && creature[cS].recoil == 0) {
...
cutscene[0].lev.wallPanels.push(new wordWall(410,270, 'EXAM',5));
	...
	function Trap(a,b,p, play){
		...
		if(play){//Trap is for player eyes only
			result.collideCreature = function(c){
				return c == creature[cS] && (c.collidePoint(this.x,this.y) || c.collidePoint(this.x + 100,this.y) || c.collidePoint(this.x,this.y+ 30) || c.collidePoint(this.x + 100,this.y + 30))
			}
		}
	...
	function createCreature(a,b,c,d){
		...
		var result = {
			...
			stats:{speed:10, jump:10, health:100, maxHealth:100, armor:0, arm:0, torso:0, legs:0, hair:1, abdomen:0, hasArms:true, wings:0, type:0}, //type 0- Insect, 1-Reptile
			weapon:wArray, 
			sw:0,
			light:true,
			aimer:{x:0,y:0},
			...
			move:function(){
				...
				//to do: path finding wing vs no wing
				
				//Confirm target is valid
				if(this.target >= creature.length || this.retarget == 0) this.target = -1;
				...
				if(this.target == -1){
					...
					var bestD = -1;
					for(var i=0; i < creature.length;i++){
						if(creature[i].stats.type != this.stats.type && creature[i].stats.health > 0 /*&& Math.abs(creature[i].y - this.y) < 100*/){//Found a target to attack
							//this.target = i;
							if(dist(creature[i].x, creature[i].y, this.x, this.y) < bestD || bestD < 0){
								bestD = dist(creature[i].x, creature[i].y, this.x, this.y)
								bestInd = i
							}
						}
					}
					...
					if(this.target != -1) {
						...
						this.path = aStar({x:Math.floor((this.x + 100)/100), y:Math.floor((this.y + 100)/100)} , {x:  Math.floor((creature[this.target].x + 100)/100),y:Math.floor((creature[this.target].y + 100)/100)   }, this.stats.wing>0);
				...
				//Confirm target is not allied
					if(creature[this.target].stats.type == this.stats.type) this.target = -1;
					if(creature[this.target].stats.health <= 0){
						this.target = -1
						this.speak("Target Down!");
						this.path = [];
						//this.taimer = {x:this.x +100, y:this.y + 200};
					}
				}
				...
				if(this.target != -1){
				//Target is valid
					this.taimer.x = creature[this.target].x + 100
					this.taimer.y = creature[this.target].y + 100
					...
					if(this.path.length == 0){
							if(this.x - creature[this.target].x < -50) {
								this.state = 2;
								this.sx = this.stats.speed;
							}else if(this.x - creature[this.target].x > 50){
								this.state = 3;
								this.sx = this.stats.speed * -1;
							}
							
							if(dist(this.x, this.y, creature[this.target].x, creature[this.target].y) > 100){
								var start = {x:Math.floor((this.x + 100)/100), y: Math.floor((this.y + 100)/100)}
								var end = {x:Math.floor((creature[this.target].x + 100)/100), y: Math.floor((creature[this.target].y + 100)/100)}
								
								if(mapGrid[start.x][start.y].wall ) start = this.getCorner();
								if(mapGrid[end.x][end.y].wall ) end = creature[this.target].getCorner();
							...
							if(dist(this.x + 100, this.y + 100, this.path[0].x*100 + 50, this.path[0].y*100 + 50) < 50) this.path.splice(0,1);
							else{//Move to path 0
								if(this.WillFire() != this.target && this.target >= 0 || this.weapon[this.sw].type != 1){
	
								if(this.x + 100 - this.path[0].x*100 -50< -10){
									this.state = 2;
									this.sx = this.stats.speed;
								}else if(this.x + 100 - this.path[0].x*100 -50 > 10){
									this.state = 3;
									this.sx = this.stats.speed * -1;
								...
								if(this.y + 100 - this.path[0].y*100 - 20 > 10)	this.jump();
								...
								if(dist(this.x + 100, this.y + 100, this.path[0].x*100 + 50, this.path[0].y*100 + 50) > 150){
									var start = {x:Math.floor((this.x + 100)/100), y: Math.floor((this.y + 100)/100)}
									var end = {x:Math.floor((creature[this.target].x + 100)/100), y: Math.floor((creature[this.target].y + 100)/100)}
					...
					if((this.weapon[this.sw].type == 0 || this.weapon[this.sw].type ==3) && this.target>= 0){//melee selected
						...
						if(dist(this.x+100, this.y+100, creature[this.target].x+100, creature[this.target].y + 100) <= this.weapon[this.sw].stats.range) {
							if(this.fireCool <= 0 && this.canFire && this.recoil == 0)this.fire();
							
							if(dist(this.x+100, this.y+100, creature[this.target].x+100, creature[this.target].y + 100) <= 30) {
							//Too close, back up!
								if(this.facing == 0){
									this.state = 3;
									this.sx = this.stats.speed * -1;
						...
						if(this.weapon.length >= 4){
							if(!this.weapon[2].useless && this.weapon[2] != null && this.weapon[3] != null){
								if(Math.random() > 0.5){
					...
					if(this.path.length == 0 && this.target != -1){
						...
						if(dist(this.x, this.y, creature[this.target].x, creature[this.target].y) < 250){//too close
							if(creature[this.target].x < this.x){
						...
						}else if(dist(this.x, this.y, creature[this.target].x, creature[this.target].y) > 400 && this.stats.type == creature[cS].stats.type){//too close
							if(creature[this.target].x < this.x){
						...
						}else if(this.fireCool <= 0 && this.canFire && this.recoil == 0){
							//Fire ranged weapon
							if(this.target != -1){
								if(dist(this.x, this.y, creature[this.target].x, creature[this.target].y) < w/2 && this.WillFireGeneral()){
				...
				if(this.target == -1){
					...
					if(dist(creature[cS].x, creature[cS].y, this.x,this.y) < 250 && this.stats.type == creature[cS].stats.type){
						if(this.x < creature[cS].x - 100){
							this.sx = 5
							this.state = 2;
						}else if(this.x > creature[cS].x +100){
							this.sx = -5
							this.state = 3;
						}
						this.aimer.x = creature[cS].aimer.x
						this.aimer.y = creature[cS].aimer.y
					}
			...
			drawBody:function(fNum, x,y){
				...
				if(this.stats.type == 0){	//Insect
					...
					if(this.stats.health > 0){
						...
						//ctx.restore();
						if(ani == 0) this.deathF += 2;
						if(this.deathF > 10) this.deathF = 10;
						else{//Extra blood splatters for death sequence, not working quite right yet
							//bloodSplat.push(createBlood(this.x  +this.hitBox.x + rand(this.hitBox.width), this.y + this.hitBox.y + rand(this.hitBox.height) + this.deathF*9));
			...
			die:function(){
				for(var i=0; i < this.weapon.length; i++) this.dropWeapon(i);
			},
			dropWeapon:function(i){
				if(i >= 0 && i < this.weapon.length && this.weapon[i] != null){
			...
			draw:function(){
				...
				for(var i=0; i < this.path.length; i++){
					ctx.lineTo(this.path[i].x*100 + 50, this.path[i].y*100 + 50);
			...
			//Returns a corner on the hitbox not currently impacting any walls or stuck in mapGrid
				...
				if(mapGrid[Math.floor((this.x + this.hitBox.x)/100)][Math.floor((this.y + this.hitBox.y)/100)].wall == false) {
					result = {x:Math.floor((this.x + this.hitBox.x)/100), y:Math.floor((this.y + this.hitBox.y)/100)};
					//console.log("Top left corner is fine.");
			}else if(mapGrid[Math.floor((this.x + this.hitBox.x + this.hitBox.width)/100)][Math.floor((this.y + this.hitBox.y)/100)].wall == false) result = {x:Math.floor((this.x + this.hitBox.x + this.hitBox.width)/100), y:Math.floor((this.y + this.hitBox.y)/100)};
			...
			WillFireGeneral:function(){ //x and y are the destination of the projectile
				...
				for(var i =0; i < results.length; i++){
					if(results[i].t == this.stats.type) {
						results.splice(i,1)
						i--;
					}
				}
	...
	function pulseLight(a,b){
		...
		this.rTime = 40
		this.distance = 0
		this.sound = pulseLightSound[rand(pulseLightSound.length)]
		this.draw = function(){
			if(this.timer <= 0) {
			...
			if(this.distance < 300) {
				this.sound.play();
				this.sound.setVolume ((1- this.distance / 300)/10); //has a 50% drop in volume
				if(this.sound.soundVar.currentTime > this.sound.soundVar.duration - 0.5) this.sound.soundVar.currentTime = 0.1
			}else this.sound.stop();
	...
	function spinLight(a,b,c,d){
		...
		this.CSize = c;
		this.angle = d;
		this.sound = spinSound[rand(spinSound.length)]
		this.distance = 0;
		this.draw = function(){
			...
			if(this.distance < 200) {
				this.sound.play();
				this.sound.setVolume ((1- this.distance / 200)/100); //has a 50% drop in volume
				if(this.sound.soundVar.currentTime > this.sound.soundVar.duration - 0.5) this.sound.soundVar.currentTime = 0.1
			}else this.sound.stop();
		}
	}
	...
	function steamJet(a,b){
		...
		this.draw = function(){
			...
			if(this.distance < 200) {
				this.sound.play();
				this.sound.setVolume ((1- this.distance / 200)/5); //has a 50% drop in volume
				if(this.sound.soundVar.currentTime > this.sound.soundVar.duration - 0.5) this.sound.soundVar.currentTime = 0.1
			}else this.sound.stop();
		}
...
window.addEventListener('keyup', function(evt){
	...
	function sparker(a,b){
		...
		this.length = 20;
		this.pause = 0;
		this.sound = sparkSound[rand(sparkSound.length)]
		this.distance = 0;
	...
	function getNeighborsNoFlight(x,y){
		...
		for(var i= x-1; i <= x+1; i++){
			for(var j= y-1; j <= y; j++){
				if(i >= 0 && i < mapGrid.length && y>=0&&y< mapGrid[0].length&& i!=x && j != y){
					if(mapGrid[i][j].wall == false)	result.push(newNode({x:i,y:j}));
				}
			}
		}
...
level[2].wallPanels.push(new ForeGround(650,200, door[0], '', 0.5));
level[2].wallPanels.push(new ForeGround(950,200, door[1], '', 0.5));
level[2].wallPanels.push(new ForeGround(1300,200, door[0], '', 0.5));
...
level[3].wallPanels.push(new ForeGround(1700,850, door[0], '', 0.5));level[3].foreGround.push(new ForeGround(1040,950, foreGPic[0]));
...
level[4].wallPanels.push(new ForeGround(500,200, door[0], '', 0.5));
...
level[5].wallPanels.push(new ForeGround(500,400, wallFeatures[28]));
level[5].wallPanels.push(new ForeGround(500,500, wallFeatures[30]));
level[5].foreGround.push(new ForeGround(570,500, foreGPic[0]));
level[5].foreGround.push(new ForeGround(670,500, foreGPic[1]));
level[5].wallPanels.push(new ForeGround(600,400, wallFeatures[24]));level[5].wallPanels.push(new ForeGround(800,400, wallFeatures[26]));
...
level[5].wallPanels.push(new ForeGround(1000,500, wallFeatures[30]));
level[5].wallPanels.push(new ForeGround(1100,500, wallFeatures[31]));
level[5].wallPanels.push(new ForeGround(1200,500, wallFeatures[31]));
level[5].wallPanels.push(new ForeGround(1000,400, wallFeatures[32]));
level[5].wallPanels.push(new ForeGround(1100,400, wallFeatures[32]));
...
level[5].wallPanels.push(new Panel(900,600, panelHall[1], 0,0.5));level[5].wallPanels.push(new wordWall(1020,860, 'REPTILE',5));level[5].wallPanels.push(new wordWall(1020,870, 'SPECIMENS',4));level[5].wallPanels.push(new ForeGround(1110,900, wallFeatures[22]));level[5].wallPanels.push(new ForeGround(1650,800, door[0], '', 0.5));
level[5].wallPanels.push(new ForeGround(1450,800, door[0], '', 0.5));
level[5].wallPanels.push(new ForeGround(1250,800, door[0], '', 0.5));
...
level[5].wallPanels.push(new ForeGround(550,800, door[1], '', 0.5));
level[5].wallPanels.push(new ForeGround(350,800, door[1], '', 0.5));
level[5].wallPanels.push(new ForeGround(150,800, door[0], '', 0.5));
...
level[5].items.push(pistolItem(420,270));level[5].wallPanels.push(new wordWall(110,880, 'OBSERVATION',4));
level[5].wallPanels.push(new wordWall(120,890, 'LOUNGE',4));
...
MAke some creatures have a speed of 3 - 5 no decimals
--> upgrade left / right movement sections use the new value
...
scientist melee weapon, and pistol sequence

work in the ally system and selector UI

Creatures need faces

creature class needs controllable attribute

need first tiny creature

need first super basic killable mutant

key press UI to make allied unit join player squad
